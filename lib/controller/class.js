...
module.exports = SuperJS.Api.Controller.extend({
  ...
  search: function(resolve, reject, req) {
    ...
    req.parameters.joins = (typeof req.parameters.joins === 'object') ? req.parameters.joins : {};

    var searchPromise = self.model.find();

    searchPromise.where(req.parameters.where);

    for( var join in req.parameters.joins ) {
      searchPromise.populate(join, req.parameters.joins[join]);
    }
